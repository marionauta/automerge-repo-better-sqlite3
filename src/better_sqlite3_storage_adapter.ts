...
export class BetterSqlite3StorageAdapter implements StorageAdapterInterface {
  ...
  constructor(database: Database, tableName: string = "automerge_repo_data") {
    this.#db = database;
    ...
    this.#save_stmt = this.#db.prepare(
      `INSERT INTO ${tableName} (key, updated_at, data) VALUES (?, datetime(), ?);`,
    );
    this.#remove_stmt = this.#db.prepare(
      `DELETE FROM ${tableName} WHERE key = ?;`,
    );
    this.#loadRange_stmt = this.#db.prepare(
      `SELECT * FROM ${tableName} WHERE key GLOB ?;`,
    );
  ...
  async load(keyArray: StorageKey): Promise<Uint8Array | undefined> {
    ...
    const result = this.#load_stmt.get(key) as
